...
class Scheduler:
    ...
    def _schedule_prefills(
        ...
        while self._passed_delay(time.time()) and waiting_queue:
            ...
            num_lookahead_slots = self._get_num_lookahead_slots_for_seq_group(
                seq_group, enable_chunking)
    ...
    def _get_num_lookahead_slots_for_seq_group(
        self,
        seq_group: SequenceGroup,
        enable_chunking: bool,
    ) -> int:
        """
        Calculates the number of lookahead slots for a specific sequence group,
        considering per-sequence K values. This centralized logic prevents
        inconsistencies between can_append and append operations.
        """
        is_prefill = seq_group.is_prefill()
        # Get the per-sequence K value for this specific group.
        sg_next_k = getattr(seq_group, 'num_speculative_tokens', 0)

        # 1. Decode phase logic
        if not is_prefill:
            if sg_next_k > 0:
                spec_config = self.scheduler_config.speculative_config
                max_possible_k = 0
                if spec_config:
                    if spec_config.dynamic_speculation and hasattr(spec_config, "max_spec_tokens"):
                        max_possible_k = spec_config.max_spec_tokens or 0
                    else:
                        max_possible_k = spec_config.num_speculative_tokens or 0
                lookahead_k = max(sg_next_k, max_possible_k)
                return lookahead_k + 1
            # For non-speculative (normal) decode:
            if self.scheduler_config.is_multi_step and self.scheduler_config.num_lookahead_slots > 0:
                # Non-speculative multi-step lookahead.
                return self.scheduler_config.num_lookahead_slots
            # Standard decode requires 1 slot for the next token.
            return 1

        # 2. Prefill phase logic
        if self.scheduler_config.is_multi_step and enable_chunking:
            # For chunked prefill, prepare for the next decode step.
            # Use the per-sequence K if available.
            _base_k = self.scheduler_config.num_lookahead_slots
            if sg_next_k > 0:
                spec_config = self.scheduler_config.speculative_config
                max_possible_k = 0
                if spec_config:
                ...
                _base_k = max(sg_next_k, max_possible_k)
            
            num_lookahead_slots = _base_k
            if num_lookahead_slots >= 0:
                num_lookahead_slots += 1
            return num_lookahead_slots

        # 3. Default case (e.g., normal prefill without chunking)
        return 0
    ...
    def _can_append_slots(self, seq_group: SequenceGroup,
        ...
        # Get the required slots using the single, reliable helper function.
        num_lookahead_slots = self._get_num_lookahead_slots_for_seq_group(
            seq_group, enable_chunking)

        # The rest of the function is identical to the vLLM original.
        is_prefill = seq_group.is_prefill()
    ...
    def schedule(
        ...
        for i, scheduled_seq_group in enumerate(
            ...
            if hasattr(seq_group, 'num_speculative_tokens'):
                setattr(seq_group_metadata, 'num_speculative_tokens', getattr(seq_group, 'num_speculative_tokens'))
    ...
    def get_seq_group_from_seq_id(self, seq_id: int) -> Optional[SequenceGroup]:
        """Gets the sequence group from a sequence id by checking all queues."""
        # Check running sequences first as it's the most likely place.
        for sg in self.running:
            if seq_id in sg.seqs_dict:
                return sg
        ...
        # Check swapped sequences as a last resort.
