...
class RejectionSampler(SpecDecodeStochasticBaseSampler):
    ...
    def __init__(self,
        ...
        self._last_accepted_mask: Optional[torch.Tensor] = None
        self._last_proposal_lens: Optional[torch.Tensor] = None
        self._last_kl_tensor: Optional[torch.Tensor] = None
    ...
    @property
    def last_accepted_mask_per_sequence(self) -> Optional[torch.Tensor]:
        return self._last_accepted_mask

    @property
    def last_proposal_lens_per_sequence(self) -> Optional[torch.Tensor]:
        return self._last_proposal_lens

    @property
    def last_kl_tensor(self) -> Optional[torch.Tensor]:
        return self._last_kl_tensor
    ...
    def forward(
        ...
        if self.use_flashinfer and chain_speculative_sampling is not None:
            ...
            self.num_emitted_tokens += emitted_token_num.sum()
            self.num_draft_tokens += torch.sum(proposal_lens).item()
            self._last_accepted_mask = None
            self._last_proposal_lens = proposal_lens
        ...
        # Dynamic K Scheduler
        self._last_accepted_mask = accepted
        self._last_proposal_lens = proposal_lens

        # KL Divergence
        try:
            from vllm.spec_decode.dynamic_scheduler import compute_kl_divergence
            
            # draft_prob / target_with_bonus_probs is the full set of tokens from draft and target models.
            # Size is (batch size, proposal token count, vocab size). -1 excludes the bonus token.
            kl_tensor = compute_kl_divergence(
                draft_probs=draft_probs,
                target_probs=target_with_bonus_probs[:, :-1, :])

            if kl_tensor is not None:
                valid_mask = torch.arange(k_max, device=kl_tensor.device)[None, :] < proposal_lens[:, None]
                kl_tensor[~valid_mask] = 0.0
            self._last_kl_tensor = kl_tensor
        except Exception as e:
            logger.error("KL compute failed: %s", e, exc_info=True)
            self._last_kl_tensor = None

        return output_token_ids

    def _batch_modified_rejection_sampling(
        self,
        target_probs: torch.Tensor,  # [batch_size, k_max, vocab_size]
        draft_probs: torch.Tensor,  # [batch_size, k_max, vocab_size]
        draft_token_ids: torch.Tensor,  # [batch_size, k_max]
        ...
        Returns:
            ...
            0: A bool tensor of which tokens in each sequence is accepted.
                shape = [batch_size, k_max]
            1: Token ids sampled from a recovered distribution, to be used
                when a token is rejected.
                shape = [batch_size, k_max]
        """

        batch_size, k_max, vocab_size = draft_probs.shape

        # shape [batch_size, k]
        accepted = self._get_accepted(target_probs, draft_probs,
                                    draft_token_ids, proposal_lens, seeded_seqs)

        recovered_probs = self._get_recovered_probs(
            target_probs, draft_probs).reshape(batch_size * k_max, vocab_size)
    ...
    def _get_accepted(
        ...
        probs_indicies = torch.arange(k_max, device=target_probs.device)
        ...
        # Per-sequence-masking
        valid_mask = torch.arange(k_max, device=draft_probs.device)[None, :] < proposal_lens[:, None]
    ...
    def _create_output(
        ...
        batch_size, k_max = accepted.shape
        ...
        accepted_padded = torch.cat(
            [accepted, torch.zeros((batch_size, 1), dtype=torch.bool, device=device)], dim=1
        )
        accepted_len = torch.argmin(accepted_padded.int(), dim=1)

        pos_indices = torch.arange(k_max, device=device).expand_as(accepted)
        prefix_mask = pos_indices < accepted_len.unsqueeze(1)
        output_token_ids[:, :k_max] = torch.where(
            prefix_mask, draft_token_ids, VLLM_INVALID_TOKEN_ID
        )

        for i in range(batch_size):
            k_i = proposal_lens[i].item()
            acc_len = accepted_len[i].item()
            if acc_len < k_i:
                output_token_ids[i, acc_len] = recovered_token_ids[i, acc_len]
            elif k_i > 0:
                output_token_ids[i, k_i] = bonus_token_ids[i, 0]
        # Metric update
        num_accepted = accepted.sum()
        num_emitted = (output_token_ids != VLLM_INVALID_TOKEN_ID).sum()
        num_draft = proposal_lens.sum()

        self.num_accepted_tokens += num_accepted.item()
        self.num_emitted_tokens += num_emitted.item()
        self.num_draft_tokens += num_draft.item()
