...
class LLMEngine:
    ...
    def __init__(
        ...
        self.scheduler_config.speculative_config = self.speculative_config
        ...
        self.next_k_map_from_worker: Dict[str, int] = {}
    ...
    def _create_sequence_group_with_sampling(
        ...
        if self.speculative_config:
            initial_k = (getattr(self.speculative_config,
                                 'initial_num_speculative_tokens', 0) or
                         self.speculative_config.num_speculative_tokens or 0)

            if self.speculative_config.num_speculative_tokens:
                draft_size = self.speculative_config.num_speculative_tokens + 1
        seq_group = SequenceGroup(
            ...
            initial_speculative_k=initial_k
        )
    ...
    def _apply_worker_k_updates(self, virtual_engine: int) -> None:
        """Apply the K-values from the previous step's worker output to the
        sequence groups in the scheduler's queues.
        """
        if not (self.speculative_config and self.next_k_map_from_worker):
            return

        queues_to_update = [
            self.scheduler[virtual_engine].waiting,
            self.scheduler[virtual_engine].running
        ]
        for queue in queues_to_update:
            for sg in queue:
                if sg.request_id in self.next_k_map_from_worker:
                    new_k = self.next_k_map_from_worker[sg.request_id]
                    if getattr(sg, 'num_speculative_tokens', None) != new_k:
                        sg.num_speculative_tokens = new_k
        
        # Initialize after k updated.
        self.next_k_map_from_worker = {}

    def _store_worker_k_updates(self, execute_model_req: ExecuteModelRequest) -> None:
        """Store the K-values for the next step, which are returned from the
        worker after model execution.
        """
        if (execute_model_req.updated_k_values_for_scheduler is not None and
                len(execute_model_req.updated_k_values_for_scheduler) > 0):
            self.next_k_map_from_worker = \
                execute_model_req.updated_k_values_for_scheduler
    ...
    def step(self) -> List[Union[RequestOutput, PoolingRequestOutput]]:
        ...
        # iteration
        self._apply_worker_k_updates(virtual_engine)
        ...
        if not self._has_remaining_steps(
            ...
            if (self.scheduler_config.is_multi_step and
                    scheduler_outputs and scheduler_outputs.num_lookahead_slots > 0):
        ...
        if not scheduler_outputs.is_empty():
            ...
            self._store_worker_k_updates(execute_model_req)
            if self.scheduler_config.is_multi_step:
        ...
        if self.scheduler_config.is_multi_step:
            for seq_group_meta_obj in seq_group_metadata_list:
                seq_group_meta_obj.finish_step()
        ...
        if not self._has_remaining_steps(seq_group_metadata_list):
            ...
            is_first_step_output: bool = False
            if seq_group_metadata_list:
                if hasattr(seq_group_metadata_list[0], 'state') and \
                   seq_group_metadata_list[0].state is not None:
                    is_first_step_output = \
                        seq_group_metadata_list[0].state.num_steps == 1
                elif seq_group_metadata_list[0].is_prompt:
