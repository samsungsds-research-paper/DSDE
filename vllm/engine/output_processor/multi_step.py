...
class MultiStepOutputProcessor(SequenceGroupOutputProcessor):
    ...
    def process_outputs(self,
        ...
        metrics_obj = getattr(sequence_group, 'metrics', None)
        counts_list = getattr(metrics_obj, 'spec_token_acceptance_counts', None)

        if isinstance(counts_list, list):
            for output in outputs:
                if not (output.samples and
                        output.samples[0].output_token != VLLM_INVALID_TOKEN_ID):
                    continue

                step_idx = output.step_index
                if step_idx >= len(counts_list):
                    counts_list.extend([0] * (step_idx - len(counts_list) + 1))
                counts_list[step_idx] += 1
