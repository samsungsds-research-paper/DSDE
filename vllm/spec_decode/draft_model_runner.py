    ...
    def execute_model(
        ...
        **kwargs,
        ...
        for step in range(num_steps):
            ...
            # Token-wise ============================================
            Lambda_tensor = getattr(self, "Lambda_tensor", None)
            gamma_risk_tensor = getattr(self, "gamma_risk_tensor", None)
            min_k = int(getattr(self, "k_min", 1))
            steps_done = len(outputs)

            if Lambda_tensor is not None and gamma_risk_tensor is not None:
                # initializing.
                device = logits.device
                Lambda = Lambda_tensor.to(device=device, dtype=torch.float32).flatten()
                gamma  = gamma_risk_tensor.to(device=device, dtype=torch.float32).flatten()

                # entrophy calculating.
                probs = torch.softmax(logits, dim=-1, dtype=torch.float32)
                h_dm = (-(probs * (probs + 1e-9).log()).sum(dim=-1))

                # effective length arrange.
                B, L, G = h_dm.shape[0], Lambda.shape[0], gamma.shape[0]
                valid_len = min(B, L, G)
                if valid_len == 0:
                    condition_met = torch.tensor(False, device=device)
                else:
                    h_dm  = h_dm[:valid_len]
                    Lambda = Lambda[:valid_len]
                    gamma  = gamma[:valid_len]

                    prod = torch.clamp(gamma * h_dm, min=0.0)
                    score = 1.0 - torch.sqrt(prod)
                    condition_met = (score < Lambda).any()

                if steps_done >= min_k and condition_met:
                    break
            # ===========================================================
